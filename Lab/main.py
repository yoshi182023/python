try:
    ...
    from Lab.task_utils import add_task, calculate_progress, mark_task_as_complete, tasks, view_pending_tasks
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            try:
                pending_index = int(input("Enter the pending task number to mark complete: ").strip()) - 1
            except ValueError:
                print("Invalid task number.")
                continue

            if pending_index < 0 or pending_index >= len(pending_tasks):
                print("Task number out of range.")
                continue

            actual_index = tasks.index(pending_tasks[pending_index])
